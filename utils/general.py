...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,labels=(), kpt_label=False, nc=None, nkpt=5):
# def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False,multi_label=False,labels=(), kpt_label=False, nc=None, nkpt=17):
    ...
    #xc[1,25500] xc用于保存一些布尔值，高于约定值的位置为True，低于约定值的位置为False
...
def non_max_suppression_export(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    #prediction.shape = [1, 25500, 57]
    #确定类别 这里kpt_lable =True  nc = 57 - 56 = 1
    if nc is None:
        nc = prediction.shape[2] - 5  if not kpt_label else prediction.shape[2] - 56 # number of classes
        # nc = prediction.shape[2] - 5 if not kpt_label else prediction.shape[2] - number_of_keypoint *3 -5
    min_wh, max_wh = 2, 4096  # (pixels) minimum and maximum box width and height
    xc = prediction[..., 4] > conf_thres  # candidates
...
def increment_path(path, exist_ok=False, sep='', mkdir=False):
    ...
    #如果路径存在 并且 exist_ok 为假
    if path.exists() and not exist_ok:
        #获取‘路径中的’文件后缀名
        suffix = path.suffix
        #删除文件路径扩展名 eg:/path/to/file.txt -> /path/to/file
        path = path.with_suffix('')
        #glob.glob()查找与path相似的文件路径或者目录路径
        #eg: paht = path/to/file sep = '/'  f{path}{sep}* 表示 path/to/file/目录下所有文件 *为通配符 表示任何字符或子目录
        dirs = glob.glob(f"{path}{sep}*")  # similar paths
        #path.stem 返回文件名 但不包括文件扩展后缀 eg:path=path/file.txt path.stem= file runs\detect path.stem=detect
        #rf"%s{sep}(\d+)" % path.stem 后面的%是格式化占位符 表示将被path.stem替换
        #在路径dir中寻找和rf"%s{sep}(\d+)" % path.stem 匹配的路径 re.search返回匹配对象 否则返回None
        #寻找带增量的路径
        matches = [re.search(rf"%s{sep}(\d+)" % path.stem, d) for d in dirs]
        # 取出matches中匹配到的所有路径中的序号 eg: exp2 -> 2
        i = [int(m.groups()[0]) for m in matches if m]  # indices
        #如果i存在 则用最大的索引i+ 否则使n=2
        #获取增量文件索引n
        n = max(i) + 1 if i else 2  # increment number
        #更新路径
        path = Path(f"{path}{sep}{n}{suffix}")  # update path
        #path后缀不为空 则dir = path父目录 否则dir = path
    dir = path if path.suffix == '' else path.parent  # directory
    #目录不存在 则创建一个目录
    if not dir.exists() and mkdir:
        # parents 为真 表明上级目录不存在也会一并创建
        dir.mkdir(parents=True, exist_ok=True)  # make directory
    return path
