...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', tidl_load=False ,kpt_label=True,kpt_num=5):
...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32):
        #将路径转化为绝对路径后 转换为str类型
        p = str(Path(path).absolute())  # os-agnostic absolute path
        #若绝对路径P中含有通配符*
        if '*' in p:
            #在路径中查找所有含有path路径的文件，排序后放到files追踪
            #recursive=True 表明递归得获取P目录及其子目录下的所有文件
            '''
            path
             |_file1.txt
             |_sub1_dir
             ...
             |_sub2_dir
                 file3.txt
            recursive = True可以递归地获取P目录及其子目录下所有文件
            '''
            files = sorted(glob.glob(p, recursive=True))  # glob
        #若绝对路径P中不含通配符 并且是一个路径
        #将p路径下所有文件返回并排序后，赋值给files
        #*.* 是一个通配符，表示无所谓文件的文件名和扩展名，往路径后面加就完事了
        elif os.path.isdir(p):
            files = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        #如果p是一个文件，则将其加入到files列表中
        ...
        #img_formats = ['bmp', 'jpg', 'jpeg', 'png', 'tif', 'tiff', 'dng', 'webp', 'mpo']  # acceptable image suffixes
        #vid_formats = ['mov', 'avi', 'mp4', 'mpg', 'mpeg', 'm4v', 'wmv', 'mkv']  # acceptable video suffixes
        #取files文件列表中的文件扩展名，转换成小写并查看是否在指定的后缀中
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, stride=32):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            #s.isnumeric()：这是一个字符串方法，用于检查字符串 s 是否只包含数字字符。如果 s 中的所有字符都是数字字符（0 到 9），则返回 True，否则返回 False
            #如果 s 是一个数字字符串（只包含数字字符），则 eval() 函数将尝试将其转换为相应的数字类型。例如，如果 s 是字符串 "42"，eval(s) 将返回整数 42。
            # s：如果 s 不是一个纯数字字符串（包含非数字字符），则不执行 eval()，而直接将 s 保留在原样。
            s = eval(s) if s.isnumeric() else s  # i.e. s = '0' local webcam
            #打开一个视频文件或者从摄像头实时获取视频流
            ...
            '''
                cap.read() 从cap中读取一帧图像。返回两个值：
                第一个返回一个布尔值，表示是否成功读取了一帧图像，第二个返回一帧图像
            '''
            #保证第一帧
            _, self.imgs[i] = cap.read()  # guarantee first frame
            #在线程中处理视频流的更新 [i,cap]表示帧索引和视频流对象(cv2.VideoCapture) daemnon=True 表示线程设置为守护进程
            #该守护进程会在后台对视频不断处理为一帧帧图片并放到img[]数组中
    ...
    def update(self, index, cap):
        ...
        n = 0
        #如果视频流成功打开，cap.isOpened() 返回 True，否则返回 False。
        while cap.isOpened():
            n += 1
            # _, self.imgs[index] = cap.read()
            #用于在视频流中获取下一帧，但不直接获取图像数据。它通常用于在缓冲区中准备下一帧图像，以提高读取效率。该方法返回 True 表示成功，False 表示失败。
            cap.grab()
            if n == 4:  # read every 4th frame
                #这是 cv2.VideoCapture 对象的方法，用于获取 grab() 方法准备的图像数据
                #该方法返回一个元组 (success, image)，其中 success 是一个布尔值，表示是否成功获取图像，image 是获取到的图像数据（通常是一个 NumPy 数组）。
                success, im = cap.retrieve()
                #secess为True 将im赋值给imgs[index] 否则将imgs[index]置为0
                self.imgs[index] = im if success else self.imgs[index] * 0
                n = 0
            #等待一定时间 控制帧的更新速度 等待1/self.fps秒
            # time.sleep(1 / self.fps)  # wait time
            time.sleep(1 / 30)
    #迭代行为特殊方法，通常与__next__迭代器一起使用
    def __iter__(self):
        #迭代器初始化为-1
        self.count = -1
        return self
    #迭代器 特殊方法 通常与迭代行为__iter__ 特殊方法一起使用
    def __next__(self):
        #迭代器自增1
        self.count += 1
        #创建一个图像副本Img0
        ...
        # Letterbox 在满足步幅倍数约束的情况下 调整和填充图像大小
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix='',square=False, tidl_load=False, kpt_label=True,kpt_num=5):
        self.img_size = img_size
        ...
        self.flip_index = [0, 2, 1, 4, 3, 6, 5, 8, 7, 10, 9, 12, 11, 14, 13, 16, 15][:kpt_num]
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        #断言语句检查条件是否为真，如果为假 引发Assertionror异常 同时指定一个自定义的错误消息
                        #.all()方法检查l中所有元素是否都大于0 如果l可迭代，all方法在l中素有元素大于0时返回True，否则返回False
                        assert (l >= 0).all(), 'negative labels'
                        if kpt_label:
                            # assert l.shape[1] == 56, 'labels require 56 columns each'
                            #l:tuple(1,20) l.shape[0]= 1 l.shape[1]=20
                            assert l.shape[1] >=5 + 2 * self.kpt_num, 'labels require 56 columns each'
                            ...
                            #创建了一个NumPy数组 kpts,kpts 的形状是 (l.shape[0], 39)，其中 l.shape[0] 表示 l 数组的行数，即与 l 数组的行数相同，而39表示39列。数组的元素都被初始化为0。
                            #kpts[1,39]
                            # kpts = np.zeros((l.shape[0], 39)) 39 = 2*kpt_num(17) + 5
                            #15 = 2 *kpt_num(5) + 5
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    #img是一张图片，目前的shape代表[height，width，channel]
    shape = img.shape[:2]  # current shape [height, width]
    #如果newshape是一个整数，将其转换成两个相同的数组成的数组，用以表示一个正方形的高和宽
    ...
    #如果scaleup的值为假 说明只需要下采样 取r 或者 1之间的最小值
...
def random_perspective(img, targets = (), segments = (), degrees = 10, translate = .1, scale = .1, shear= 10,
                       perspective = 0.0,
                       border= (0, 0), kpt_label = False, kpt_num = 5) :
