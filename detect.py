...
def detect(opt):
    ...
    # Directories
    #保存的地址 并且地址存在的时候 会提供增量地址 eg: exp存在 则用exp2保存
    save_dir = increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok)  # increment run
    #save_txt 或者 save_txt_tidl 任一为真 在save_dir下创建一个label文件用于存放label 否则只返回save_dir
    ...
    #判断模型是否是列表[]或者元祖() 列表可变 元祖不可变 两者都可通过下表访问数据
    #是列表或元祖的情况，可以用于图片长宽尺寸不同时
    if isinstance(imgsz, (list,tuple)):
        #如果imgsz长度不为2 则抛出异常 assert condition, message condition为假  引发AssertionError异常 并提供message信息
    ...
    #hasattr判断model是否具有'module'属性 有则names =  mode.module.names 没有则names = model.name
    ...
    if webcam:
        ...
        #作用是告诉 cuDNN 是否应该在每个批次中自动寻找最适合当前硬件和输入数据的卷积算法。
        cudnn.benchmark = True  # set True to speed up constant image size inference
        #如果采用摄像头获取数据 则加载来自摄像头的实时图像流 并允许进行实时推理
    ...
    if device.type != 'cpu':
        #对模型进行一次前向传播
        #to(device)将张量从一个设备移到另一个设备
        #type_as将一个张量转换为另一个给定张量相同数据类型的方法
        #next() 是一个内置的 Python 函数，用于从迭代器中获取下一个元素
        model(torch.zeros(1, 3, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # run once
    t0 = time.time( )
    #取文件
    for path, img, im0s, vid_cap in dataset:
        #.from_numpy 方法将 NumPy 数组转换为 PyTorch 张量

        #再将torch张量放入设备.to(device)
        img = torch.from_numpy(img).to(device)
        #half=True ->数据图像转换为半精度浮点数
        img = img.half() if half else img.float()  # uint8 to fp16/32
        #数据像素值归一化 0-255 -> 0.0-1.0
        img /= 255.0  # 0 - 255 to 0.0 - 1.0
        #如果数据维度为3 插入一个新的维度到图像数据最前作为batch维度
        if img.ndimension() == 3:
            img = img.unsqueeze(0)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--weights', nargs='+', type=str, default=r'F:\workwork\edgeai-yolov5-yolo-pose\runs\train\exp10\weights\best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str,default=r'F:\workwork\edgeai-yolov5-yolo-pose\runs\train\exp6\weights\best.pt',help='model.pt path(s)')
    ...
    print(opt)
    # #检测系统中是否已经安装所需的Python包或者库 exclude参数用于指定不需要检查的包或者库
